...
{//departamento,ip,marca,modelopc,nserie,numeropatrimonio,nomemaquina,patrimonioprov,sigla,estado,datacadastro,ultimaalteracao
    public class DALComputador
    {
        ...
        {//---------------------------------------------------------------------------------------------------------------------INCLUIR
            ...
            cmd.CommandText =
"insert into computadores(departamento,ip,marca,modelopc,nserie,numeropatrimonio,nomemaquina,patrimonioprov,sigla,estado,datacadastro,ultimaalteracao) " +
"values (@departamento,@ip,@marca,@modelopc,@nserie,@numeropatrimonio,@nomemaquina,@patrimonioprov,@sigla,@estado, @datacadastro,@ultimaalteracao); select @@IDENTITY;"; //Defini qual comando ele vai executar           
        ...
        {//---------------------------------------------------------------------------------------------------------------------ALTERAR
            ...
            cmd.CommandText = 
                "update computadores set numeropatrimonio = (@numeropatrimonio), departamento = (@departamento),ip = (@ip),marca = (@marca), "+
                "modelopc = (@modelopc), nserie = (@nserie), nomemaquina = (@nomemaquina), patrimonioprov = (@patrimonioprov), sigla = (@sigla), "+
                "estado = (@estado), ultimaalteracao = (@ultimaalteracao) where codigo = (@codigo);";
        ...
        {//---------------------------------------------------------------------------------------------------------------------Localizar
            ...
            MySqlDataAdapter da = new MySqlDataAdapter("Select * from computadores where numeropatrimonio like '%" +
        ...
        public DataTable LocalizarGambiarra()
        {//---------------------------------------------------------------------------------------------------------------------GAMBIARRA
            DataTable tabela = new DataTable();
            MySqlDataAdapter da = new MySqlDataAdapter("Select * from computadores ", conexao.StringConexao);
            da.Fill(tabela);
            return tabela;
        }
